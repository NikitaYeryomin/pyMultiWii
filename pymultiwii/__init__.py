...
class MultiWii:
    ...
    def __init__(self, host, port):
        ...
        try:
            self.sock.connect((host, port))
            if self.PRINT:
                print ("Connected to the board on " + host + ":" + str(port))
        except Exception as error:
            print("\n\nError connecting to " + host + ":" + str(port))
            print(str(error) + "\n\n")
        
        #"wakeup", not sure it's needed with SITL
        time.sleep(2)
    ...
    def sendCMD(self, data_length, code, data, data_format):
        ...
        try:
            ...
            b = self.sock.send(struct.pack('<3c2B'+ data_format + 'B', *total_data))
        except Exception as error:
            print ("\n\nError in sendCMD.")
            print ("("+str(error)+")\n\n")
            pass
    ...
    def sendCMDreceiveATT(self, data_length, code, data):
        ...
        try:
            start = time.time()
            b = self.sock.send(struct.pack('<3c2B%dHB' % len(data), *total_data))
            
            while True:
                header = self.sock.recv(1).decode('utf-8')
                if header == '$':
                    header = header + self.sock.recv(2).decode('utf-8')
                    break

            datalength = struct.unpack('<b', self.sock.recv(1))[0]
            code = struct.unpack('<b', self.sock.recv(1))
            data = self.sock.recv(datalength)
            temp = struct.unpack('<'+'h'*int(datalength/2),data)
            
            # Instead of flushing (since we're using sockets now), you might consider implementing your own buffer or manage the data you've read.
    ...
    def setVTX(self,band,channel,power):
        ...
        different = (self.vtxConfig['band'] != band) or (self.vtxConfig['channel'] != channel) or (self.vtxConfig['power'] != power)
    ...
    def connect(self):
        try:
            ...
            print("\n\nError connecting to " + self.host + ":" + str(self.port))
            print(str(error) + "\n\n")
    ...
    def getData(self, cmd):
        try:
            ...
            while True:
                header = self.sock.recv(1).decode('utf-8')
                # print('DEBUG: header: %s' % header)
                if header == '$':
                    header = header + self.sock.recv(2).decode('utf-8')
                    break

            datalength = struct.unpack('<b', self.sock.recv(1))[0]
            # print('DEBUG: datalength: %s' % datalength)
            code = struct.unpack('<b', self.sock.recv(1))
            data = self.sock.recv(datalength)
            # print('DEBUG: data: %s' % data)

            # Remove serial flush since sockets don't have this method
            elapsed = time.time() - start
    ...
    def getDataInf(self, cmd):
        while True:
            try:
                ...
                # The lines flushing the serial buffer are removed since they don't apply to sockets
                if cmd == MultiWii.ATTITUDE:
                    self.attitude['angx'] = float(temp[0] / 10.0)
                    self.attitude['angy'] = float(temp[1] / 10.0)
                    self.attitude['heading'] = float(temp[2])
    ...
    def getData2cmd(self, cmd):
        try:
            ...
            # First Command: ATTITUDE
            self.sendCMD(0, self.ATTITUDE, [])
            while True:
                header = self.sock.recv(1).decode('utf-8')
                if header == '$':
                    header += self.sock.recv(2).decode('utf-8')
                    break
            datalength = struct.unpack('<b', self.sock.recv(1))[0]
            code = struct.unpack('<b', self.sock.recv(1))
            data = self.sock.recv(datalength)
